...
export class SCComponentReader extends Reader{
    ...
    async readXMLFile(localeFile, ordered) {
        return this.readFile(localeFile).then(data => this.parseXML(data,ordered)).catch(e => {
            console.log("error in " + localeFile + " " + e)
        })
    }
    ...
    async read(scopesArray,catalogs){
        ...
        if(scopes.data){
            ...
            let commonFiles
            if(catalogs){
                commonFiles = catalogs.map(el => el + "data.xml");
            }
            else{
                commonFiles = SCGame.datafiles.map(el => el + "data.xml");
            }
            ...
            for (let file of includes) {
                ...
                if (data) {
                    datacatalogs.push({id: file, data: data.Catalog?.constructor === Object ? data : {}});
                }
            }
            ...
            let entities = []
            for (let catalog of datacatalogs) {
