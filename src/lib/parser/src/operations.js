...
export function resolveSchemaType(schema,property,pathobject,schemaForResolvedArrays = false){
    ...
    //typ can ddepend on other field value type used in userdata and triggers
...
export function _addRelation({target,namespace, link, patharray, type, result, ignorelist}){
    ...
    let WHY_NEED_TO_IGNORE_FILTER_RELATIONS = false

    if(WHY_NEED_TO_IGNORE_FILTER_RELATIONS){
...
export function getValueEntity(target,refpath){
    if (!refpath) return null //nothing to rename
    let _path = refpath.split(".")
    let value, valueObject, valueProperty, valueEntity
    let referenceEntity = target;//this.cache[_path[0]][_path[1]]
    if (!referenceEntity) {
    ...
    return {value, valueObject, valueProperty, valueEntity, _path}
}
